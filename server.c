...
void send_message(mbedtls_net_context *network_client, mbedtls_aes_context *aes, unsigned char * shared_secret)
{
    int ret;
    size_t olen;
...
int main()
{
    ...
    send_message(&network_client, &aes, &shared_secret);

    recv_message(&network_client, &aes, &shared_secret);
